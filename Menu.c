...
struct Kiet_Airline
{
	...
	int seat_num;
	char email[15];
	struct Kiet_Airline *following;
}
*begin, *stream;
struct Kiet_Airline *dummy;
...
void main()
{
	...
	do
	{
		...
		printf("\n\n\t\t ********************************************************************");
		printf("\n\t\t                   welcome to Kiet Airline system                   ");
		printf("\n\t\t   ********************************************************************");
...
void reserve(int x)
{
	...
	if (begin == NULL)
	{
		begin = stream = (struct Kiet_Airline*)malloc(sizeof(struct Kiet_Airline));
		details();
	...
	else
	{
		...
		stream->following = (struct Kiet_Airline *)malloc(sizeof(struct Kiet_Airline));
		stream = stream->following;
		details();
		stream->following = NULL;
